...
The `dsp-keys/` directory must exist under `common-operating-picture/` and contain:
...
> `make dev-certs` requires [mkcert](https://github.com/FiloSottile/mkcert).
...
All commands run from `common-operating-picture/`:

### Start

```bash
docker compose -f docker-compose.dsp-only.yaml up --build
```

To run detached (in the background):

```bash
docker compose -f docker-compose.dsp-only.yaml up --build -d
```

### Stop

```bash
docker compose -f docker-compose.dsp-only.yaml down
```

To also delete the Postgres volumes:

```bash
docker compose -f docker-compose.dsp-only.yaml down -v
```
...
```bash
# All services
docker compose -f docker-compose.dsp-only.yaml logs -f

# Single service
docker compose -f docker-compose.dsp-only.yaml logs -f dsp
docker compose -f docker-compose.dsp-only.yaml logs -f keycloak
```
...
```bash
docker compose -f docker-compose.dsp-only.yaml ps
```
...
- Check Keycloak-DB is healthy: `docker compose -f docker-compose.dsp-only.yaml ps keycloak-db`
...
- Check DSP logs: `docker compose -f docker-compose.dsp-only.yaml logs dsp`

### Provisioning container exits non-zero

- Check logs: `docker compose -f docker-compose.dsp-only.yaml logs dsp-keycloak-provisioning`
- Re-run provisioning manually after DSP is healthy:
  ```bash
  docker compose -f docker-compose.dsp-only.yaml run --rm dsp-keycloak-provisioning
  ```
...
DSP_KEYCLOAK_DB_PORT=25435 docker compose -f docker-compose.dsp-only.yaml up
```
...
Get a token as the new user and check which DSP attribute values they are entitled to:
...
curl -fks -H "Authorization: Bearer $USER_TOKEN" \
  https://local-dsp.virtru.com:8080/entitlements | jq .
```
...
```bash
docker compose -f docker-compose.dsp-only.yaml run --rm dsp-provision-federal-policy
```
