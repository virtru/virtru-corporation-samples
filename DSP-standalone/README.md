...
Generate them using `mkcert` from the `DSP-standalone/` directory:

```bash
# Install mkcert if needed
brew install mkcert && mkcert -install

# Generate TLS cert for the local hostname
mkcert \
...
KAS keys (`kas-cert.pem`, `kas-private.pem`, `kas-ec-cert.pem`, `kas-ec-private.pem`) are
generated by the scripts in `common-operating-picture/` (`.github/scripts/init-temp-keys.sh`
and `.github/scripts/x509-temp-keys.sh`). Run `make dev-certs` from that directory and copy
the resulting key files into `DSP-standalone/dsp-keys/`.
